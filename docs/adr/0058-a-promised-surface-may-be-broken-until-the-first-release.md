...
- **#339 — a replaced `apply-adjustments` or `calculate-tax` may no longer drop an Adjustment it
  was handed.** Extension Point 2, and **the first break here that no compiler announces**, which
  ...
  **`calculate-tax` carries the same guard, and the reason it needed one is the register's own
  ...
  on compiling there and losing the same delivery charge. Both slots are guarded as of this
  entry, with the same matching and different advice in the message. A shape that compels an
  acknowledgement is not a shape that compels a value to survive, and that distinction is the
  thing to carry forward rather than the two slots.

  **What actually stops working is two shapes, and a Project could honestly have written
  either**: at `apply-adjustments`, a Step that implements free delivery by *removing* the
  shipping Adjustment rather than by adding a discount beside it; and at `calculate-tax`, a Step
  that taxes the **lines** — because lines are what tax is about — and rebuilds the value without
  the Order's own Adjustments. That is the tightening, and it is taken on ADR-0022's own
  ...
  it was given, `0` included, which is what Core's own `calculate-tax` does.

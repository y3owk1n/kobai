...
describe("a declared refusal is one a gate actually makes", () => {
  ...
  it("puts no permission gate above the session gate that feeds it", () => {
    ...
    // The fifteen routes that name a permission — every admin route but `POST /admin/session`,
    // which mints the session the other fifteen are read through, and `GET`/`DELETE

...
// The one definition of what this repository does when a teardown meets a directory
// something else is still writing into. Reached by a path rather than by a package name
// because `tests/` is not one; that module's header says why importing it here is an
// ordinary dependency of this program rather than a second `include` of the same file.
import { removeAll } from "../../../tests/support/removal.ts";
...
/**
 * Removes every workspace this file made, and fails naming any that would not go.
 *
 * **This is where #313 was found**, and the whole of what it settled — retries on the
 * transient family, `Promise.allSettled` so one stuck directory takes none of the others, and
 * an `AggregateError` naming each one with `rm`'s errno kept as its `cause` — lives in
 * `removeAll`. The two files under `tests/` that had the same bug call the same function, so
 * there is one definition of the policy rather than a copy per caller.
 *
 * What is still writing here is git's own doing rather than a subprocess this file forgot to
 * wait for: `scaffold()` awaits all three of its git invocations and every test awaits
 * `scaffold()`. `git commit` ends by spawning a *detached* `git maintenance run --auto`, which
 * repacks `.git/objects` after the command that started it has exited — the
 * `ENOTEMPTY: directory not empty, rmdir '/tmp/kobai-scaffold-…/corner-shop/.git/objects'`
 * that failed #312's run in teardown with all thirteen of this file's tests already green.
 * `removeAll` says the rest of it.
 */
afterAll(() => removeAll(workspaces));

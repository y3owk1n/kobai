...
/**
 * Rewrites the one occurrence of `find`, and refuses if there is not exactly one.
 *
 * `compose.yaml` is YAML and nothing in this package parses YAML, so the adaptation below is
 * a string edit. What makes that safe is this refusal: `String.replace` on an anchor that has
 * moved returns the input **unchanged**, and an adaptation that silently does nothing is the
 * worst failure available here — the drift test would compare a template generated by the
 * broken generator against a template written by the broken generator, agree, and ship a
 * Project with no volume again. Failing loudly instead surfaces it the first time anyone
 * regenerates.
 */
function replaceOnce(contents: string, find: string, replacement: string): string {
  const parts = contents.split(find);
  if (parts.length !== 2) {
    throw new Error(
      `An adaptation anchors on ${JSON.stringify(find)} and the text it was given contains it ${parts.length - 1} times rather than once. Re-anchor it against the file as it now reads — and note that an adaptation applied after another sees what that one produced, so the text may not be the reference Project's bytes unaltered.`,
    );
  }
...
/**
 * The two places the Media volume is written into the generated `compose.yaml`, and the
 * anchors it is written against.
 *
 * Up here rather than inline because they are the part that goes stale: they are bytes of
 * the reference Project's compose file, quoted, and `replaceOnce` is what turns a change
 * underneath them into a failure rather than a silent no-op.
 */
const APP_DEPENDS_ON = `    depends_on:
      db:
        condition: service_healthy
`;

const TOP_LEVEL_VOLUMES = `
volumes:
  kobai-db:
`;

const MEDIA_VOLUME_MOUNT = `    volumes:
      # This Store's images, and everything else a Merchant uploads. The storage kobai ships
      # — what a Project that configures no \`media\` storage gets — writes them to
      # \`kobai-media/\` beside the application *inside* the container, and a container's own
      # filesystem goes away with the container. Without this line every image in this
      # catalog is gone the next time you rebuild and bring this up, and nothing tells you
      # until a page renders without them.
      #
      # A named volume rather than a bind mount, for the same reason \`kobai-db\` is one. The
      # application runs as an unprivileged user inside the container, and Docker populates a
      # fresh named volume from the image's own directory, ownership included — whereas a
      # host directory Docker has to create arrives owned by root, and the first upload is
      # refused. It also keeps the bytes off whatever filesystem this Project happens to be
      # checked out on. \`docker compose down\` keeps them; \`docker compose down --volumes\`
      # deletes them, along with the database.
      #
      # Point \`media\` at an object store in \`kobai.config.ts\` and no bytes come here at all:
      # this volume is then holding nothing, and can go.
      - kobai-media:/app/kobai-media
`;
...
export function adaptationsFor(context: AdaptationContext): readonly Adaptation[] {
  return [
    ...
    {
      file: "compose.yaml",
      what: "the volume a Merchant's uploads survive a redeploy in, which a Project deployed for real needs and the reference Project must not have — kobai's release gate uploads test fixtures (ADR-0029), and a volume there would persist nothing but the leftovers of somebody's run",
      apply: (contents) =>
        replaceOnce(
          replaceOnce(contents, APP_DEPENDS_ON, `${MEDIA_VOLUME_MOUNT}${APP_DEPENDS_ON}`),
          TOP_LEVEL_VOLUMES,
          `${TOP_LEVEL_VOLUMES}  kobai-media:\n`,
        ),
    },
  ];
}

  ...
  try {
    ...
    process.stdout.write(
      [
        ...
        "",
        "That brings up Postgres and the app, applies every migration on boot, and serves",
        "the API on http://localhost:3000 with the Admin at /admin-ui.",
        "",
        "Docker is the only prerequisite. If you also have devbox, `devbox run up` does the",
        "same thing, and `devbox run install` then `devbox run dev` runs the app on your",
        "machine against just the database.",
        "",
      ].join("\n"),
    );
    return 0;

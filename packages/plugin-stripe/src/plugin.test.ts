 ...
 *
 * This is the second Plugin in this repository that owns a table, and the first that owns one
 * for something Core deliberately does not model: money that arrived and produced no Order
 * (ADR-0070). That makes the questions below the same ones, asked of a package whose reason for
 * having a table is different — which is the useful thing about asking them twice.
 */
describe("a Plugin owns its own tables", () => {
  it("creates exactly one table, carrying its own prefix, once a Project wires it", async () => {
    ...
    await expect(schema.tablesOwnedBy("stripe")).resolves.toEqual([
      "stripe_unplaced_refund",
    ]);
  });
  ...
  it("tracks its migrations in its own table, in the explicitly named schema", async () => {
    ...
    expect(tracking.map((entry) => `${entry.schema}.${entry.table}`)).toEqual([
      ...
      "drizzle.__drizzle_migrations_plugin_stripe",
    ]);
    ...
    const declared = await declaredMigrations(stripeMigrationSet);
    expect(declared.length).toBeGreaterThan(0);
    await expect(appliedMigrations(kobai.database, stripeMigrationSet)).resolves.toEqual(
      declared,
    );
  });
  ...
  it("scopes its migration config to its own table prefix", () => {
    ...
    expect(drizzleConfig.tablesFilter).toEqual(["stripe_*"]);
    expect(drizzleConfig.migrations).toEqual({
      table: "__drizzle_migrations_plugin_stripe",
      schema: "drizzle",
    });
  });
  ...
  it("has no foreign key pointing into a Core table", async () => {
    ...
    // ADR-0004. This Plugin's `cart_id` is Core's row by ID and nothing more — and here the
    // rule earns its keep twice over, because the Cart a refund names may well have been
    // swept away by the time a Merchant reads the books. A constraint would have deleted the
    // account of the money along with it.
    await expect(
...
describe("an installed Plugin does nothing until the Project wires it", () => {
  it("brings no table into being when the Project has not named its migration set", async () => {
    ...
    expect(stripeMigrationSet.name).toBe("plugin-stripe");
...
describe("install order is not a hidden constraint", () => {
  it("applies before Core's set as readily as after it", async () => {
    ...
    // which is why it is asked here rather than trusted from the Plugins next door.
    ...
    // nothing conflicted.
    await expect(
      inspectSchema(backwards.database).tablesOwnedBy("stripe"),
    ).resolves.toEqual(["stripe_unplaced_refund"]);
    for (const set of [coreMigrationSet, stripeMigrationSet]) {
